...
// Direct URL digunakan sebagai fallback lokal (dev mode)
const DIRECT_URL = `https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL}:generateContent`;
// Proxy URL — Vercel serverless function, API key tersimpan di server
...
// ─── Gemini Request — proxy-first, direct fallback ────────────────────────────
// Tries the Vercel serverless proxy first (key stays server-side).
// Falls back to direct call with user's stored API key for local dev or
// when the proxy has no key configured.

const geminiRequest = async (requestBody, userApiKey) => {
  // 1. Try proxy (production: key is server-side, users never see it)
  try {
    ...
    // 404 = proxy not deployed (local dev) → fall through
    // NO_SERVER_KEY = proxy deployed but env var not set → fall through
    if (proxyRes.status !== 404 && errData.code !== 'NO_SERVER_KEY') {
      throw new Error(errData.error?.message || errData.error || `Proxy error ${proxyRes.status}`);
    }
  } catch (e) {
    // TypeError = network error (proxy not reachable) → fall through to direct
  ...
  const res = await fetch(`${DIRECT_URL}?key=${userApiKey}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(requestBody),
  });
  ...
  if (!res.ok) {
    ...
    throw new Error(b.error?.message || `Gemini API Error ${res.status}`);
  }
  return res.json();
};
...
export const parseTextWithAI = async (textContent, fileName, onProgress) => {
  ...
  const result = await geminiRequest({
    contents: [{ parts: [{ text: `${UMKM_PROMPT}\n\nIsi file "${fileName}":\n\n${truncated}` }] }],
    generationConfig: GEN_CONFIG,
  }, getStoredApiKey());
...
export const parsePDFWithAI = async (file, onProgress) => {
  ...
  const aiResult = await parseWithAI(text, getStoredApiKey(), onProgress);
